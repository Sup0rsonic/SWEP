...
class Spider():
    ...
    def FetchHomepage(self):
        try:
            resp = requests.get(str('%s://%s/' %(self.Protocol, str(self.Url))), timeout = int(self.Timeout)).text
            InitalPages = self.GetLink(resp)
            print '[+] Fetched %s page from homepage.' %(len(InitalPages))
            return InitalPages
        except requests.Timeout:
            while raw_input('[*] SWEP got an timeout fetching homepage. Retry?(Y/n) ').upper() != 'N':
                self.FetchHomepage()
        except Exception, e:
